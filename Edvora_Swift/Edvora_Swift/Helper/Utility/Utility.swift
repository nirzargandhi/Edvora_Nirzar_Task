...
struct Utility {
    ...
    func setRootSignInVC() {
        ...
        let navhomeViewController = UINavigationController(rootViewController: objSignInVC)
...
func userDefaultKeyChainDataClear() {
    ...
    KeychainWrapper.standard.removeObject(forKey: UserDefault.kUsername)
...
func makeAttributedString(strTitle : String, strValue : String, colorValue : UIColor? = .black, isValueStringBold : Bool? = false) -> NSAttributedString {
    ...
    let attributeTitle = [NSAttributedString.Key.foregroundColor: UIColor.black, NSAttributedString.Key.font: UIFont.init(name: "OpenSans-Bold", size: 13)]
    
    let attributeValue = [NSAttributedString.Key.foregroundColor: colorValue, NSAttributedString.Key.font: isValueStringBold ?? false ? UIFont.init(name: "OpenSans-Bold", size: 13) : UIFont.init(name: "OpenSans-Regular", size: 13)]
...
//MARK: - Get Device Information Method
func getDeviceInformation() -> String {
    ...
    return "Apple / \(model) / \(modelName) / \(systemName) / \(systemVersion) / \(Bundle.main.versionNumber).\(Bundle.main.buildNumber)"
}
